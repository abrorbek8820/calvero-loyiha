...
function Home() {
  ...
  const [showConfirm, setShowConfirm] = useState(false); // ✅ yangi modal holati
  const location = useLocation();

  // Har safar sahifaga qaytilganda animatsiyani qayta ishga tushirish
  useEffect(() => {
  ...
  const buttons = [
    ...
    { text: "IJARA UYLAR", link: "https://kvartira.calvero.work/uy-kerak" },
    { text: "IJARA UY BERISH", link: "https://kvartira.calvero.work/my-houses" },
  ];
  ...
  const handleClick = (index, link) => {
    ...
    setTimeout(() => {
      // 🏠 Kvartira subdomenlari — WebView ichida ochiladi
      if (link.includes("kvartira.calvero.work")) {
        const encoded = encodeURIComponent(link);
        navigate(`/kvartira?url=${encoded}&mode=${mode}`);
      }
      // 💼 ISH KERAK — modal orqali tasdiq
      else if (link.includes("/ishkerak")) {
        setShowConfirm(true);
      }
      // 👷 ISHCHI KERAK — WebView ichida (normal routing)
      else {
        navigate(link);
      }
    }, 700);
  ...
  <div className="modal">
    ...
    <button
      onClick={() => {
        ...
        setClickedIndex(null); // 🔁 animatsiyani qayta ishga tushirish
      }}
    >
      Davom etish
    </button>
